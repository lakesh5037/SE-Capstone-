pipeline {
    ...
    stages {
        ...
        // ─── STAGE 4: DEPLOY CONTAINERS WITH PERSISTENT VOLUMES & SCHEMA ───────
        stage('Deploy & Run Application Containers') {
            steps {
                echo 'Deploying HemoScan containers with persistent storage and importing SQL schema...'
                sh '''
                    ...
                    # 3. Start Database on hemoscan-net with PERSISTENT volume (data never gets wiped)
                    ...
                    # 4. Wait for MySQL server to be ready
                    echo "Waiting for MySQL database server..."
                    sleep 8
                    ...
                    # 5. Initialize database & import SQL schema atomically
                    ...
                    # 6. Start Backend on hemoscan-net with PERSISTENT uploads volume
    ...
    post {
        ...
        success {
            echo '''
            ======================================================================
            🎉 SUCCESS: HemoScan stack deployed with permanent volume persistence!
